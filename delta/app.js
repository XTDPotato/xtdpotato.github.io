...
function initScrollers() {
    // 先检查 scrollCols 是否有有效元素
    if (!scrollCols || scrollCols.length === 0) {
        console.error('未找到滚动列元素');
        return;
    }
    ...
    scrollElements.forEach((el, index) => {
        ...
        if (!col) {
            console.error(`索引 ${index} 处的滚动列元素不存在`);
            return;
        }
        
        // 检查 dataset 属性是否存在
        if (!col.dataset || !col.dataset.chars || !col.dataset.target) {
            console.error(`滚动列元素缺少必要的 data 属性: ${JSON.stringify(col.dataset)}`);
            return;
        }
...
selectButton.addEventListener('click', () => {
    ...
    if (!currentCol) {
        console.error(`当前索引 ${currentColIndex} 处的滚动列不存在`);
        showToast('操作失败，请刷新页面重试');
        return;
    }
    
    // 检查必要的 dataset 属性
    if (!currentCol.dataset || !currentCol.dataset.target || !currentCol.dataset.chars || !currentCol.dataset.next) {
        console.error(`当前滚动列缺少必要的 data 属性`);
        showToast('操作失败，请刷新页面重试');
        return;
    }
...
window.addEventListener('load', async () => {
    ...
    if (dataLoaded) {
        // 再次确认滚动列元素已加载
        const scrollColsCheck = document.querySelectorAll('.scroll-container');
        if (scrollColsCheck.length === 0) {
            console.error('未找到任何滚动列元素，请检查HTML结构');
